...
class FindGraspsAction(AbstractAction):
    ...
    def run(self, segmented_obj):
        ...
        try:
            grasps = self._suggest_grasps_srv(cloud=segmented_obj.point_cloud).grasp_list.poses
            if len(grasps) == 0:
            ...
            grasps = self._grasps_rank_srv().grasp_list.poses
            if len(grasps) == 0:  # Something has gone horribly wrong
                raise Exception("Pairwise reranking of the grasps failed!")
            elif self._stopped:
                yield self.set_preempted()
            else:
                yield self.set_succeeded(grasps=grasps[:self._max_grasps])
